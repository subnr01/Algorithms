...
if (isset($_GET["name"]) && isset($_GET["type"])) {
    ...
    if($type=="All"){
	    ...
	    $result = mysql_query("SELECT * FROM `Restaurant` WHERE `name`='$name'") or die(mysql_error());
    }
    else{
	// get all products from products table
	$result = mysql_query("SELECT * FROM `Restaurant` WHERE `name`='$name' AND `type`='$type'") or die(mysql_error());
	}
