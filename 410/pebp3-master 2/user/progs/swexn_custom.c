...
{
    lprintf("stack array %p, stack_top %p", &exn_staq, EXN_STAQ_TOP );

	int ret;
    lprintf("arg %p at address %p", arg, &arg);
    lprintf("uregs %p at address %p", uregs, &uregs);
    ...
    lprintf("new pages for %p", GOOD_MEMORY);
