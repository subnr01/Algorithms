...
void __attribute__((noreturn)) slaughter(char *file, unsigned int depth)
{
  ...
  lprintf("=====> Start slaughter %d!", curtid);

  if ( ! depth )
  {
    char *argv[] = { file, 0 };
    lprintf("=====> execing %d!", curtid);
    if (exec(file, argv))
    {
  ...
  else
  {
    ...
    for (i = 0; i != 2*width; ++i)
    {
      ...
      else
      {
        ...
        lprintf("=====> %stid %d gave %svalid status code of %d to tid %d, exit_status %d", 
          (status == exit_status) ? "" : "XXX ",
          tid, (status == exit_status) ? "" : "IN", status, curtid, exit_status);
